...
/**
 * This class stands for ValidateToken StructType
 * @subpackage Structs
 * @generated on Tue, 07 Nov 17 12 44
 */
class ValidateToken extends AbstractStructBase
{
    /**
     * The sSessionKey
    ...
    public $iDataroomID;
    /**
     * Constructor method for ValidateToken
     * @uses ValidateToken::setSSessionKey()
     * @uses ValidateToken::setGuidToken()
     * @uses ValidateToken::setIDataroomID()
     * @param string $sSessionKey
     * @param string $guidToken
     * @param int $iDataroomID
     */
    public function __construct($sSessionKey = null, $guidToken = null, $iDataroomID = null)
    {
        $this
            ->setSSessionKey($sSessionKey)
            ->setGuidToken($guidToken)
     ...
     * @param string $sSessionKey
     * @return \Brainloop\Soap\StructType\ValidateToken
     */
     ...
     * @return \Brainloop\Soap\StructType\ValidateToken
     */
    public function setGuidToken($guidToken = null)
    {
        // validation for constraint: pattern
        if (is_scalar($guidToken) && !preg_match('/[\\da-fA-F]{8}-[\\da-fA-F]{4}-[\\da-fA-F]{4}-[\\da-fA-F]{4}-[\\da-fA-F]{12}/', $guidToken)) {
            throw new \InvalidArgumentException(sprintf('Invalid value, please provide a scalar value that matches "[\da-fA-F]{8}-[\da-fA-F]{4}-[\da-fA-F]{4}-[\da-fA-F]{4}-[\da-fA-F]{12}", "%s" given', var_export($guidToken, true)), __LINE__);
        }
        // validation for constraint: string
        if (!is_null($guidToken) && !is_string($guidToken)) {
            throw new \InvalidArgumentException(sprintf('Invalid value, please provide a string, "%s" given', gettype($guidToken)), __LINE__);
        }
